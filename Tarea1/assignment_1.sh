...
for i in $splited
    ...
    #Without indels (-I). Output .bcf (-g). Automatic index .fai generation (-f)
    samtools mpileup -Igf raw_data/$fasta b_alignment/$basename.bam >  c_variant_calling/variants_$basename.bcf
